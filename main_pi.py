...
class Config:
    ...
    # --- Timing Strategy ---
    UPLOAD_INTERVAL = 300.0   # Upload setiap 5 Menit (Agregasi)
...
class RadarPiSystem:
    def __init__(self):
        ...
        # Buffer Agregasi (Untuk Rata-rata 5 Menit)
    ...
    def send_data_worker(self, payload):
        ...
        if self.supabase:
            try:
                ...
                print(f"[CLOUD] Data Terkirim: Dist={payload['distance']}m | Vel={payload['velocity']}m/s")
            except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            print(f"[INFO] Radar Aktif (Fixed Mode 2). Upload tiap {Config.UPLOAD_INTERVAL} detik.")
            sleep(1)
        except Exception as e:
        ...
        chirp_rate_calc = 20.0 # Default awal
        ...
        try:
            while self.running:
                ...
                # Hitung Real Sampling Rate (Penting untuk akurasi kecepatan)
                sample_counter += 1
                BATCH_SIZE_CALC = 20
                ...
                # Simpan snapshot raw data sesekali
                ...
                if len(complex_history_buffer) == Config.HISTORY_LENGTH:
                    ...
                    # Gunakan chirp_rate_calc yang dihitung real-time untuk akurasi
                    rate_for_fft = chirp_rate_calc 
                    t_start = t_stop # Reset timer
                    ...
                    if len(valid_indices) > 0:
                        ...
                        is_velocity_valid = True

                # ---------------------------------------------------------
                # STEP 4: AGREGASI DATA (KUMPULKAN SELAMA 5 MENIT)
                # ---------------------------------------------------------
                # Filter noise dasar sebelum masuk rata-rata
                if current_snr_val > 5.0 and vertical_height > 0.1:
                    self.aggregation_buffer["height"].append(vertical_height)
                    self.aggregation_buffer["snr"].append(current_snr_val)
                    
                    if is_velocity_valid:
                         self.aggregation_buffer["velocity"].append(velocity_final)

                # Print Status di Terminal (Biar tahu alat jalan)
                elapsed = time() - t_last_upload
                count_buf = len(self.aggregation_buffer["height"])
                print(f"\r[RUN] H:{vertical_height:.3f}m | V:{velocity_final:.3f}m/s | Buff:{count_buf} | Next Upload: {Config.UPLOAD_INTERVAL - elapsed:.0f}s ", end="")

                # ---------------------------------------------------------
                # STEP 5: UPLOAD (SETELAH 5 MENIT)
                # ---------------------------------------------------------
                if elapsed > Config.UPLOAD_INTERVAL:
                    print("\n[PROCESS] Menghitung Statistik...")
                    
                    count_data = len(self.aggregation_buffer["height"])
                    
                    if count_data > 0:
                        # Hitung Rata-rata
                        avg_height = np.mean(self.aggregation_buffer["height"])
                        avg_velocity = np.mean(self.aggregation_buffer["velocity"]) if self.aggregation_buffer["velocity"] else 0.0
                        
                        # Logika Moving Flag
                        is_moving = True if abs(avg_velocity) > 0.02 else False
                        
                        # Siapkan Raw JSON (Cuplikan data teknis)
                        technical_data = {
                            "mode": "fixed_mode2_manual_fft",
                            "chirp_rate_real": round(chirp_rate_calc, 2),
                            "sample_count": count_data,
                            "raw_i_sample": self.last_raw_snapshot["I"][:10], # Kirim 10 sampel saja biar ringan
                        ...
                        payload = {
                            ...
                            "distance": round(float(avg_height), 4),
                            "velocity": round(float(avg_velocity), 4),
                            "discharge": round(float(avg_height), 4), # Placeholder debit
                        ...
                        print("[SKIP] Buffer kosong (Sensor tertutup/Noise tinggi).")

                    # Reset Buffer & Timer
                    self.aggregation_buffer = {"height": [], "velocity": [], "snr": []}
                    t_last_upload = time()
